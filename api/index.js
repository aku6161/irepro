...
// server.ts
var wsTransport = typeof WebSocket === "function" ? WebSocket : WebSocket?.default || WebSocket?.WebSocket;
dotenv.config({ path: ".env.local" });
dotenv.config();
var supabaseUrl = process.env.SUPABASE_URL || "https://leshuihnieeehmhyxkvz.supabase.co";
var supabaseKey = process.env.SUPABASE_SERVICE_ROLE_KEY || "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imxlc2h1aWhuaWVlZWhtaHl4a3Z6Iiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc4NzgyNTkzOSwiZXhwIjoyMTAzNDAxOTM5fQ.oXARsks1k2FzON4b2Pl87v6zD66Cs7mPl_g7WDL_zN8";
...
var STORE_FILE = path.join(DATA_DIR, "irepro_db.json");
...
function formatUserId(u, index) {
  if (u.id && /^usr-\d{4}$/.test(u.id)) return u.id;
  if (index !== void 0) return `usr-${String(index + 1).padStart(4, "0")}`;
  const existingNums = db.users.map((user) => user.id).filter((id) => /^usr-\d{4}$/.test(id)).map((id) => parseInt(id.replace("usr-", ""), 10));
    ...
    try {
      ...
      if (usersNeedingSync.length > 0) {
        supabase.from("users").upsert(usersNeedingSync, { onConflict: "icNumber" }).then(
          ...
          (e) => console.error("[Supabase Auto-Sync Exception]:", e)
        );
      }
...
function addAuditLog(user, action, applicationId, details) {
  ...
  try {
    supabase.from("audit_logs").insert({
      ...
      (e) => console.error("[Supabase audit log catch]:", e)
    );
  } catch (err) {
...
function buildReplacements(templateKey, app2) {
  ...
  const appId = app2.applicationId || app2.id || "";
  return {
...
app.post("/api/documents/generate-pdf", async (req, res) => {
  try {
    ...
    const scriptData = await scriptRes.json().catch((e) => ({ error: e.message }));
